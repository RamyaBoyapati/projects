...
<body>
	<form method="get" action="AddEmploye.jsp">
		...
		<input type="submit" value="Added Employee" />
	</form>
	<c:if test="${param.empID!=null}">
		...
		<jsp:useBean id="emp" class="com.infinite.EmployeeProject.Employe" />
		...
		<c:out value="${dao.addEmploy(emp)}"></c:out>
		<c:set var="c" value="${beanDAO.searchEmploye(param.name,param.salary) }"/>
